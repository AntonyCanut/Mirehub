...
- **Role** : Panneau de preferences. Sections : Apparence (theme sombre/clair/systeme, taille et famille de police), Terminal (shell par defaut, scrollback), Claude (couleur de detection, auto-clauder), Notifications (son, verification MAJ au lancement). Persistence via IPC `window.theone.settings`.

# Reference API IPC - theOne/Workspaces

## Introduction

theOne utilise le systeme IPC (Inter-Process Communication) d'Electron pour communiquer entre le processus renderer (React) et le processus main (Node.js). L'architecture suit le modele de securite recommande par Electron :

```
Renderer (React)  →  Preload (contextBridge)  →  Main Process (Node.js)
window.theone.*        ipcRenderer.invoke()         ipcMain.handle()
```

- **contextIsolation** : active — le renderer n'a pas acces direct a Node.js
- **contextBridge** : expose une API typee sous `window.theone`
...
## API Preload (`window.theone`)

L'API est exposee dans le renderer via `contextBridge.exposeInMainWorld('theone', api)` depuis `src/preload/index.ts`. Voici la surface complete organisee par domaine :

```typescript
window.theone.terminal     // create, write, resize, close, onData, onClose
window.theone.workspace    // list, create, update, delete
window.theone.project      // list, selectDir, add, remove, scanClaude, scanInfo,
                           // checkClaude, deployClaude, checkPackages, updatePackage,
                           // writeClaudeSettings, writeClaudeMd
window.theone.fs           // readDir, readFile, writeFile, rename, delete, copy, mkdir, exists, readBase64
window.theone.git          // init, status, log, branches, checkout, push, pull, commit, diff,
                           // stash, stashPop, createBranch, deleteBranch, merge, fetch,
                           // stage, unstage, discard, show, stashList, renameBranch
window.theone.claude       // start, stop, onSessionEnd
window.theone.kanban       // list, create, update, delete, writePrompt, cleanupPrompt
window.theone.workspaceDir // init
window.theone.workspaceEnv // setup, getPath, delete
window.theone.updates      // check, install, onStatus
window.theone.settings     // get, set
window.theone.session      // save, load, clear
window.theone.notify       // (title, body) — fonction directe
```

Le type TypeScript complet est exporte en tant que `TheOneAPI` depuis `src/preload/index.ts`.
...
| `project:deployClaude` | `{ targetPath: string, force: boolean }` | `{ success: boolean, error?: string, hasExisting?: boolean }` | Deploie la config `.claude` de theOne vers un projet cible |
...
|-------|---------|-------------|-------------|
| `terminal:data` | `{ id: string, data: string }` | `theone.terminal.onData(cb)` | Sortie du terminal (stdout/stderr du PTY) |
| `terminal:close` | `{ id: string, exitCode: number, signal: number }` | `theone.terminal.onClose(cb)` | Fermeture du terminal (processus termine) |
| `claude:sessionEnd` | `{ id: string, status: string }` | `theone.claude.onSessionEnd(cb)` | Fin d'une session Claude (completed/failed) |
| `update:status` | `{ tool, scope, status, progress? }` | `theone.updates.onStatus(cb)` | Progression d'une installation de mise a jour |

**Pattern de cleanup** : chaque listener retourne une fonction de desinscription :

```typescript
const cleanup = window.theone.terminal.onData((data) => {

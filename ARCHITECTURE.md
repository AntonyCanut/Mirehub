...
- **Persistance locale** via fichier JSON (`~/.theone/data.json`) gere par `StorageService`
...
│                                                          │
│  ┌──────────────────────────────────────────┐           │
│  │ StorageService (JSON: ~/.theone/data.json)│           │
│  └──────────────────────────────────────────┘           │
│                                                          │
...
| **StorageService** (`src/main/services/storage.ts`) | Persistance JSON (`~/.theone/data.json`), singleton, CRUD workspaces/projects/kanban/settings/templates/sessions |
...
│   │   └── index.ts                   # contextBridge: expose window.theone (API par domaine)
│   │
...
│   │   │
│   │   ├── types/
│   │   │   └── global.d.ts           # Declaration window.theone
│   │   │
...
| better-sqlite3 | Overhead inutile, un fichier JSON (`~/.theone/data.json`) suffit pour le volume de donnees |
...
Le preload expose une API structuree par domaine via `contextBridge.exposeInMainWorld('theone', api)`. Le nom est **`window.theone`** (minuscule). Aucun module Node.js n'est expose directement.
...
**Fonctionnement** :
- Au demarrage, `StorageService` lit `~/.theone/data.json` et stocke les donnees en memoire
...
- **Mock preload**: `vi.stubGlobal('window', { theone: { ... } })` pour tester les stores renderer
...
2. **Variables d'environnement** :
   - `THEONE_KANBAN_TASK_ID` : ID de la tache Kanban en cours
   - `THEONE_KANBAN_FILE` : Chemin absolu vers `kanban.json`

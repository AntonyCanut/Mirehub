...
// ---------------------------------------------------------------------------
// Kanban integration (for ticket creation)
// ---------------------------------------------------------------------------

function getKanbanPath(workspaceId: string): string {
  return path.join(os.homedir(), '.mirehub', 'kanban', `${workspaceId}.json`)
}
...
function writeKanbanTasks(workspaceId: string, tasks: KanbanTask[]): void {
  ...
  fs.writeFileSync(getKanbanPath(workspaceId), JSON.stringify(tasks, null, 2), 'utf-8')
}

...
export function McpPanel({ mcpServers, settings, projectPath, workspaceName, onServersChange }: McpPanelProps) {
  ...
  const installedCatalogIds = useMemo(() => {
    ...
    // Check both project and workspace servers
  ...
  const doInstall = useCallback(async (entry: McpCatalogEntry, envValues: Record<string, string>, argsValues: Record<string, string>) => {
    ...
    // Resolve args placeholders: replace {{KEY}} with user-provided values
  ...
  return (
    <div className="claude-rules-mcp">
      ...
      {workspaceName && (
        <div className="mcp-scope-toggle">
          <div className="cs-mcp-transport-toggle">
            <button
              type="button"
              className={`cs-mcp-transport-toggle-btn${scope === 'project' ? ' cs-mcp-transport-toggle-btn--active' : ''}`}
            ...
            <button
              type="button"
              className={`cs-mcp-transport-toggle-btn${scope === 'workspace' ? ' cs-mcp-transport-toggle-btn--active' : ''}`}
      ...
      {loadingWorkspace && scope === 'workspace' ? (
              ...
              {installingEntry && (
                <div className="mcp-install-overlay" onClick={() => setInstallingEntry(null)}>
                  <form className="mcp-install-modal" onClick={e => e.stopPropagation()} onSubmit={handleConfirmInstall}>
                    ...
                    {installingEntry.argsPlaceholders && Object.keys(installingEntry.argsPlaceholders).length > 0 && (
                      <>
                        <div className="mcp-install-section-label">{t('claude.mcpConfigureArgsSection')}</div>
                        {Object.entries(installingEntry.argsPlaceholders).map(([key, placeholder]) => (
                          <div key={`arg-${key}`} className="claude-mcp-form-row">
                            ...
                            <input
                              ...
                              value={installArgsValues[key] ?? ''}
                              onChange={e => setInstallArgsValues(prev => ({ ...prev, [key]: e.target.value }))}
                    ...
                    {installingEntry.envPlaceholders && Object.keys(installingEntry.envPlaceholders).length > 0 && (
                      <>
            ...
            <div className="mcp-installed">
              ...
              {installedCount === 0 ? (
                ...
                <div className="claude-mcp-server-list">
                  {Object.entries(activeServers).map(([name, config]) => {
                        ...
                        <div className="claude-mcp-server-header">
                          <div className="claude-mcp-server-info">
                            <div className="claude-mcp-server-name">
                              ...
                              {scope === 'workspace' && <span className="mcp-server-scope-badge">{t('claude.mcpWorkspaceBadge')}</span>}
                            </div>
                            {catalogEntry && (
